...
import { getFoundItems } from "../api/item.api";
import { Link } from "react-router-dom";
import Navbar from "../components/Navbar";
import Footer from "../components/Footer";
import { FaSearch, FaPlus, FaFilter, FaEye, FaEyeSlash, FaMapMarkerAlt, FaCalendarAlt, FaTag, FaUser, FaCheckCircle, FaImage, FaArrowRight } from "react-icons/fa";
...
const FoundItems = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const res = await getFoundItems(category);
  ...
  const ItemCard = ({ item }) => (
    <div className="group bg-gradient-to-br from-gray-900 to-black border border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-700 transition-all duration-300 hover:shadow-xl hover:shadow-green-500/5">
      {/* Image Section */}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10">
            <div>
              <h1 className="text-4xl font-bold mb-2">
                Found <span className="bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent">Items</span>
              </h1>
              <p className="text-zinc-400">Items found by others in your community</p>
            </div>
              ...
              <div className="flex items-end">
                ...
                >
                  <span className="flex items-center gap-2">
                    {showResolved ? <FaEye className="text-green-400" /> : <FaEyeSlash className="text-blue-400" />}
          ...
          {loading && (
            <div className="text-center py-16">
              ...
              <p className="text-zinc-400">Loading found items...</p>
            </div>
          )}
          ...
          {!loading && filteredItems.length === 0 ? (
            <div className="text-center py-20 border-2 border-dashed border-zinc-800 rounded-xl bg-gradient-to-br from-gray-900/50 to-black/50">
              ...
              <h3 className="text-2xl font-semibold mb-3">No found items yet</h3>
              <p className="text-zinc-400 mb-8 max-w-md mx-auto">
                {searchTerm || category !== ""
                  ? "No items match your search criteria. Try different filters or search terms."
                  : "Be the first to report a found item and help someone get their belongings back!"}
              </p>
              <Link
                to="/report"
            ...
            <>
              ...
              <div className="mt-8 pt-6 border-t border-zinc-800">
                <p className="text-sm text-zinc-400">
                  ...
                  <span className="text-white font-medium">{items.length}</span> found items
